...
def read_and_plot_csv(main_csv="benchmark_results.csv", 
                     comparison_csv=None):
    ...
    print(f"[{main_name}]")
    print(f"Matrix size range: {df_main['n'].min()} to {df_main['n'].max()}")
    print(f"Average performance: {df_main['mean_gflops'].mean():.4f} GFLOPS")
    print(f"Peak performance: {df_main['mean_gflops'].max():.4f} GFLOPS (at size {df_main.loc[df_main['mean_gflops'].idxmax(), 'n']})")

    # 比較データの情報出力
    if df_comp is not None:
        comp_name = os.path.basename(comparison_csv).replace('.csv', '')
        print(f"\n[{comp_name}]")
        print(f"Matrix size range: {df_comp['n'].min()} to {df_comp['n'].max()}")
        print(f"Average performance: {df_comp['mean_gflops'].mean():.4f} GFLOPS")
        print(f"Peak performance: {df_comp['mean_gflops'].max():.4f} GFLOPS (at size {df_comp.loc[df_comp['mean_gflops'].idxmax(), 'n']})")
    ...
    plt.plot(df_main["n"], df_main["mean_gflops"],
    ...
    if df_comp is not None:
        ...
        plt.plot(df_comp["n"], df_comp["mean_gflops"],
    ...
    peak_size_main = df_main.loc[peak_idx_main, "n"]
    ...
    if df_comp is not None:
        ...
        peak_size_comp = df_comp.loc[peak_idx_comp, "n"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--main', default='benchmark_results.csv',
